package com.mphasis.secondspringapp;

import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.ComponentScan;
import org.springframework.context.annotation.Configuration;
import org.springframework.context.annotation.EnableAspectJAutoProxy;
import org.springframework.context.annotation.Primary;
import org.springframework.context.annotation.Profile;
import org.springframework.context.annotation.Scope;

import com.mphasis.secondspringapp.service.DataSource;
import com.mphasis.secondspringapp.service.MailServerConfig;
import com.mphasis.secondspringapp.service.UserRepository;
import com.mphasis.weatherapp.WeatherService;

@Configuration
@ComponentScan(basePackages = {"com.mphasis.secondspringapp"})
@EnableAspectJAutoProxy
public class SpringConfig {

    @Bean("weatherService")
    @Scope("singleton")
    public WeatherService weatherService() {
        return new WeatherService();
    }

    @Bean("mailServerConfigA")
    @Primary
    public MailServerConfig mailServerConfigA() {
        MailServerConfig mailServerConfig = new MailServerConfig();
        mailServerConfig.setIp("198.123.12.1");
        mailServerConfig.setLogin("Cobra");
        mailServerConfig.setToken("xxxxx");
        return mailServerConfig;
    }

    @Bean("mailServerConfigB")
    public MailServerConfig mailServerConfigB() {
        MailServerConfig mailServerConfig = new MailServerConfig();
        mailServerConfig.setIp("198.123.12.1");
        mailServerConfig.setLogin("jesus");
        mailServerConfig.setToken("zzzzz");
        return mailServerConfig;
    }

    // =============================
    // Exercise 4 (@Bean + @Profile)
    // =============================

    @Bean
    @Profile("dev")
    public DataSource devDataSource() {
        return new DataSource(
                "jdbc:h2:mem:devdb",
                "devUser"
        );
    }

    @Bean
    @Profile("prod")
    public DataSource prodDataSource() {
        return new DataSource(
                "jdbc:mysql://localhost:3306/proddb",
                "prodUser"
        );
    }

    @Bean
    public UserRepository userRepository(DataSource dataSource) {
        return new UserRepository(dataSource);
    }
}